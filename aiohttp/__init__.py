...
class ClientSession:
    ...
    def ws_connect(self, url, ssl=None, headers={}):  # 添加headers参数
        return _WSRequestContextManager(self, self._ws_connect(url, ssl=ssl, headers=headers))

    async def _ws_connect(self, url, ssl=None, headers={}):
        ws_client = WebSocketClient(None)
        # 合并base_headers和传入的headers
        combined_headers = dict(**self._base_headers, **headers)
        await ws_client.connect(url, ssl=ssl, handshake_request=self.request_raw, headers=combined_headers)
